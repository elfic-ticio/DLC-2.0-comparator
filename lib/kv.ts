...
function getRedis(): Redis {
  if (!_redis) {
    // Supports both Upstash-native vars and Vercel KV-style vars
    const url =
    ...
    if (!url || !token) {
      throw new Error(
        "Redis not configured. Set KV_REST_API_URL and KV_REST_API_TOKEN (or UPSTASH_REDIS_REST_URL/TOKEN)."
      );
    }
